...
class AuditRepositorySpec extends AnyFunSuite with Matchers with TestContainerForAll:
  ...
  test("create (job entry) — inserts and returns audit log entry") {
    ...
    result.messageText shouldBe "poll result"
  ...
  test("create (person entry) — creates a person first then inserts audit log with personId") {
    val result = run {
      for
        ...
        entry      = AuditLog(
                       ...
                       jobType       = None,
                       messageText   = "user chat message",
                       responseText  = "agent response",
                       toolCallsJson = Some(Json.arr()),
                       status        = InteractionStatus.Success,
                       errorMessage  = None,
                       createdAt     = Instant.now(),
                     )
    ...
    result.messageText         shouldBe "user chat message"
    result.responseText        shouldBe "agent response"
  }
    ...
    val (found, missing) = run {
      for
        ...
        entry    = AuditLog(
                     ...
                     messageText   = "find by id test",
                     responseText  = "",
                     toolCallsJson = None,
                     status        = InteractionStatus.Partial,
                     errorMessage  = Some("partial error"),
                     createdAt     = Instant.now(),
                   )
  ...
  test("listByJobType — returns entries for a job type using limit/offset") {
    val entries = run {
      for
        repoEnv <- (ZLayer.succeed(sharedPool) >>> AuditRepository.live).build
        repo     = repoEnv.get[AuditRepository]
        e1       = AuditLog(UUID.randomUUID(), None, Some("plaid_poll"), "msg 1", "",    None, InteractionStatus.Success, None,           Instant.now())
        e2       = AuditLog(UUID.randomUUID(), None, Some("plaid_poll"), "msg 2", "",    None, InteractionStatus.Failed,  Some("err"),    Instant.now())
        e3       = AuditLog(UUID.randomUUID(), None, Some("gmail_poll"), "msg 3", "",    None, InteractionStatus.Success, None,           Instant.now())
    ...
    val entries = run {
      for
        ...
        person    <- personEnv.get[PersonRepository]
                       ...
                       .provideEnvironment(ZEnvironment(sharedPool))
        auditRepo  = auditEnv.get[AuditRepository]
        e1         = AuditLog(UUID.randomUUID(), Some(person.id), None, "chat msg 1", "resp 1", None, InteractionStatus.Success, None,             Instant.now())
        e2         = AuditLog(UUID.randomUUID(), Some(person.id), None, "chat msg 2", "",       None, InteractionStatus.Partial, Some("partial"), Instant.now())
